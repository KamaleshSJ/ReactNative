...
import { Image, ImageBackground, Text, TouchableOpacity, useWindowDimensions, View,Dimensions } from 'react-native';
...
const CircularDatePicker: React.FC<CircularDatePickerProps> = ({
  ...
  const thumbSize = { width: 46, height: 46 };
  const animatedThumbStyle = useAnimatedStyle(() => {
    const { x, y } = polarToCartesian(center, center, radius, animatedAngle.value);
    return {
      position: 'absolute',
  ...
  const onGestureEvent = useAnimatedGestureHandler<PanGestureHandlerGestureEvent>({
    onStart: () => {
      ...
      thumbOpacity.value = withTiming(1.1, { duration: 100 });
    },
    onActive: (event) => {
      if (!animatedEditMode.value) return;
    ...
    onEnd: () => {
      ...
      animatedAngle.value = withTiming(snappedAngle, { duration: 150, easing: Easing.bezier(0.25, 0.1, 0.25, 1) },
        (isFinished) => {
          if (isFinished) {
  ...
  return (
    <View className="flex-1 items-center justify-center">
      <ImageBackground
        ...
        resizeMode="contain"
        imageStyle={{ width:'100%',marginTop:rV(50) }}
      ...
      >
        <View className="relative items-center justify-center" style={{ width: size, height: size }}>
          <PanGestureHandler onGestureEvent={onGestureEvent} enabled={editMode}>
            <AnimatedView style={{ width: size, height: size, backgroundColor }}>
              <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="absolute inset-0">
                ...
                <AnimatedCircle
                  ...
                  originY={center}
                />
                <Circle cx={center} cy={center} r={radius - strokeWidth - 10} fill="url(#innerGradient)" />
              </Svg>
              ...
              >
                <Image source={thumbIcon || require('../assets/images/Sanitary_pad.png')} className="w-full h-full" resizeMode="contain" />
              </AnimatedView>

              <View className="absolute inset-0 items-center justify-center px-4">
                <Text className="text-white font-bold text-xl text-center">DAY {displayDay}</Text>
                <Text className="text-white text-sm font-semibold mb-1 text-center">Chance for Pregnancy</Text>
                ...
                >
                  <Text className="text-white text-xs font-bold ">{editMode ? 'Done' : 'Edit'}</Text>
                </TouchableOpacity>
              </View>
            </AnimatedView>
          </PanGestureHandler>
